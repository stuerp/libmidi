...
/** $VER: GDSFile.cpp (2024.05.09) P. Stuer **/
...
static void Parameter2Bulk(const uint8_t * srcData, uint8_t * dstData);
static void Bytes2Nibbles (const uint8_t * srcData, size_t srcSize, uint8_t * dstData);
...
void rcp_converter_t::Convert(const gsd_file_t & gsdFile, midi_stream_t & midiStream, uint8_t mode)
{
    ...
    midiStream.WriteRolandSysEx(SC55_SYX_HDR, 0x400000, gsdFile.sysParams, 0x07, SYXOPT_DELAY);

    {
        ...
        midiStream.WriteRolandSysEx(SC55_SYX_HDR, 0x400110, VoiceReserve, _countof(VoiceReserve), SYXOPT_DELAY);
    }

    if (mode & 0x10)
        midiStream.WriteMetaEvent(MetaDataTypes::Text, "SC-55 Reverb Settings");

    midiStream.WriteRolandSysEx(SC55_SYX_HDR, 0x400130, gsdFile.reverbParams, 0x07, SYXOPT_DELAY);

    if (mode & 0x10)
        midiStream.WriteMetaEvent(MetaDataTypes::Text, "SC-55 Chorus Settings");

    midiStream.WriteRolandSysEx(SC55_SYX_HDR, 0x400138, gsdFile.chorusParams, 0x08, SYXOPT_DELAY);
...

    {
        ...
        {
            uint32_t addrOfs = 0x90 + Channel2Part[i] * 0xE0;
            uint32_t syxAddr = ((addrOfs & 0x007F) << 0) | ((addrOfs & 0x3F80) << 1);

            Parameter2Bulk(&gsdFile.partParams[i * 0x7A], Bulk);

            midiStream.WriteRolandSysEx(SC55_SYX_HDR, 0x480000 | syxAddr, Bulk, 0xE0, SYXOPT_DELAY, 0x80);
        }

        if (mode & 0x10)
            midiStream.WriteMetaEvent(MetaDataTypes::Text, "SC-55 Drum Setup");

        for (size_t i = 0; i < 2; ++i) // Each drum maps
        {
            ...
            for (size_t j = 0; j < 4; ++j)
            {
                ...
                for (uint8_t k = 0; k < 82; k++) // Each note
                    Parameter[27 + k] = drmPtr[k * 4 + j];

                Bytes2Nibbles(Parameter, sizeof(Parameter), Bulk);

                midiStream.WriteRolandSysEx(SC55_SYX_HDR, 0x490000 | syxAddr, Bulk, sizeof(Bulk), SYXOPT_DELAY, 0x80);
            }
        }
    }
...
static void Parameter2Bulk(const uint8_t * srcData, uint8_t * dstData)
{
    ...
    Part[0x05]  =  (srcData[0x13] & 0x01) << 7; // Mono/Poly Mode
    ...
    Part[0x07]  = ((srcData[0x17] & 0x0F) << 4) | ((srcData[0x18] & 0x0F) << 0); // Pitch Offset Fine
    ...
    {
        uint8_t srcPos = 0x37 + (uint8_t) i * 0x0B;
        uint8_t dstPos = 0x28 + (uint8_t) i * 0x0C;

        for (uint8_t j = 0; j < 3; ++j)
            Part[dstPos + 0x00 + j] = srcData[srcPos + 0x00 + j];

        Part[dstPos + 0x03] = (i == 2 || i == 3) ? 0x40 : 0x00; // Verified with Recomposer 3.0 PC-98

        for (uint8_t j = 0; j < 8; ++j)
            Part[dstPos + 0x04 + j] = srcData[srcPos + 0x03 + j];
    }

    Bytes2Nibbles(Part, sizeof(Part), dstData);
}
...
static void Bytes2Nibbles(const uint8_t * srcData, size_t srcSize, uint8_t * dstData)
{
    ...
    {
        dstData[i * 2 + 0] = (srcData[i] >> 4) & 0x0F;
        dstData[i * 2 + 1] = (srcData[i] >> 0) & 0x0F;
    }
}
