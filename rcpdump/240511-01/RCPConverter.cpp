...
/** $VER: RCPConvert.cpp (2024.05.10) P. Stuer **/
...
void rcp_converter_t::Convert(const buffer_t & srcData, buffer_t & dstData, const char * dstType)
{
    ...
    if (FileType < 0x10)
    {
        ...
        {
            throw std::runtime_error("Sequence file conversion failed.");
        }
    }
    else
    if (FileType < 0x20)
    {
        ...
        if (::_stricmp(dstType, "syx") == 0)
            Mode = 0x00;
        else
        if (::_stricmp(dstType, "mid") == 0)
            Mode = 0x01;
        else
            Mode = 0xFF;

        if (Mode == 0xFF)
            throw std::runtime_error(std::format("Unknown output format: {}.", dstType));
...
void rcp_converter_t::ConvertSequence(const buffer_t & rcpData, buffer_t & midData)
{
    ...
    if (RCPFile._Version == 2)
    {
        ...
        RCPFile._Comments.Assign(&SrcData[Offset + 0x060], 0x150);

        RCPFile._TicksPerQuarter     = (SrcData[Offset + 0x1E7] << 8) | SrcData[Offset + 0x1C0];
        RCPFile._Tempo               = SrcData[Offset + 0x1C1];
        RCPFile._BPMNumerator        = SrcData[Offset + 0x1C2];
        RCPFile._BPMDenominator      = SrcData[Offset + 0x1C3];
        ...
        RCPFile._TrackCount =  SrcData[Offset + 0x1E6];
 ...
 // if (rcpInf.fileVer == 3)
    {
        ...
        RCPFile._Comments.Assign(&SrcData[Offset + 0x0A0], 0x168);
        ...
        RCPFile._BPMNumerator        = SrcData[Offset + 0x020E];
        RCPFile._BPMDenominator      = SrcData[Offset + 0x020F];
...

    {
        ...
        for (uint16_t TrackIndex = 0; TrackIndex < RCPFile._TrackCount; ++TrackIndex)
        {
            ...
            t->LoopCount = (t->LoopStartOffs != 0) ? _Options._RCPLoopCount : 0;
            Curr += t->Size;
        }
    }

    if (!_Options._NoLoopExtension)
        BalanceTrackTimes(RCPTrack, RCPFile._TrackCount, RCPFile._TicksPerQuarter / 4, 0xFF);
    ...
    if (_Options._IncludeControlData)
    {
        ...
        {
            ::memcpy(FileName, RCPFile._CM6FileName.Data, RCPFile._CM6FileName.Len), FileName[RCPFile._CM6FileName.Len] = '\0';
            ...
            #ifdef _DEBUG
                ::printf("Using CM6 control file \"%.*s\", %s mode.\n", RCPFile._CM6FileName.Len, RCPFile._CM6FileName.Data, CM6File.DeviceType ? "CM-64" : "MT-32");
            #endif
                ControlTrackCount++;
            }
        #ifdef _DEBUG
            else
                ::printf("Warning: CM6 control file \"%.*s\" not found.\n", RCPFile._CM6FileName.Len, RCPFile._CM6FileName.Data);
        #endif
        }

        if (RCPFile._GSD1FileName.Len > 0)
        {
            ::memcpy(FileName, RCPFile._GSD1FileName.Data, RCPFile._GSD1FileName.Len), FileName[RCPFile._GSD1FileName.Len] = '\0';
            ...
            #ifdef _DEBUG
                ::printf("Using GSD control file \"%.*s\".\n", RCPFile._GSD1FileName.Len, RCPFile._GSD1FileName.Data);
            #endif
                ControlTrackCount++;
            }
        #ifdef _DEBUG
            else
                printf("Warning: GSD control file \"%.*s\" not found.\n", RCPFile._GSD1FileName.Len, RCPFile._GSD1FileName.Data);
        #endif
        }
        ...
        if (RCPFile._GSD2FileName.Len > 0)
        {
            ...
            #ifdef _DEBUG
                ::printf("Using GSD control file \"%.*s\" (Port B).\n", RCPFile._GSD2FileName.Len, RCPFile._GSD2FileName.Data);
            #endif
                ControlTrackCount++;
            }
        #ifdef _DEBUG
            else
                printf("Warning: GSD control file \"%.*s\" (Port B) not found.\n", RCPFile._GSD2FileName.Len, RCPFile._GSD2FileName.Data);
        #endif
        }
    }
    ...
    {
        MIDIStream.WriteMIDIHeader(1, 1 + ControlTrackCount + RCPFile._TrackCount, RCPFile._TicksPerQuarter);

//        _MIDITickPerQuarter = RCPFile._TicksPerQuarter;
    }
    ...
    if (ControlTrackCount > 0)
    {
        if (RCPFile._CM6FileName.Len > 0)
        {
            ...
            MIDIStream.WriteRolandSysEx(MT32_SYX_HDR, 0x7F0000, nullptr, 0, 0); // MT-32 Reset
    ...
    // Read the user SysEx data.
    {
        ...
        for (auto & syx : RCPFile._SysEx)
        {
            ...
            syx.Data = SrcData + Offset;
            syx.Size = GetTrimmedLength((const char *) syx.Data, DataSize, (char) 0xF7, true);
    ...
    midData.Copy(MIDIStream._Data, MIDIStream._Offs);
...
void rcp_converter_t::ConvertControl(const buffer_t & ctrlData, buffer_t & midiData, uint8_t fileType, uint8_t outMode)
{
    ...
    if (outMode & 0x01) // MIDI mode
    {
        ...
        MIDIFile.WriteMIDIHeader(0x0001, 1, 48);
    ...
    midiData.Copy(MIDIFile._Data, MIDIFile._Offs);
}
