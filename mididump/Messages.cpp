...
std::string DescribeControlChange(uint8_t d1, uint8_t d2) noexcept
{
    ...
    switch (d1)
    {
        ...
        case 0x05: ::snprintf(Line.data(), Line.size(), "Portamento Time (MSB) %d", Value); break;                                                 // Controls portamento rate to slide between 2 notes played subsequently.
        case 0x06: ::snprintf(Line.data(), Line.size(), "Data Entry (MSB) %d", Value); break;                                                      // Sets the Value for NRPN or RPN parameters.
        ...
        case 0x26: ::snprintf(Line.data(), Line.size(), "Data Entry (LSB) %d", Value); break;                                                      // Sets the Value for NRPN or RPN parameters.
        ...
        // ***
        case 0x40: ::snprintf(Line.data(), Line.size(), "Damper Pedal / Sustain Pedal %s", (Value < 64 ? "off" : "on")); break;                    // On/Off switch that controls sustain. (See also Sostenuto CC 66) 0 to 63 = Off, 64 to 127 = On
        case 0x41: ::snprintf(Line.data(), Line.size(), "Portamento %s", (Value < 64 ? "off" : "on")); break;                                      // On/Off switch 0 to 63 = Off, 64 to 127 = On
        case 0x42: ::snprintf(Line.data(), Line.size(), "Sostenuto %s", (Value < 64 ? "off" : "on")); break;                                       // On/Off switch. Like the Sustain controller (CC 64), However it only holds notes that were “On” when the pedal was pressed. People use it to “hold” chords” and play melodies over the held chord. 0 to 63 = Off, 64 to 127 = On
        case 0x43: ::snprintf(Line.data(), Line.size(), "Soft Pedal %s", (Value < 64 ? "off" : "on")); break;                                      // Lowers the volume of notes played. 0 to 63 = Off, 64 to 127 = On
        ...
        case 0x47: ::snprintf(Line.data(), Line.size(), "Sound Controller 2 (Timbre /Harmonic Intensity) %d", Value); break;                       // Allows shaping the Voltage Controlled Filter (VCF). Default = Resonance - also(Timbre or Harmonics)
        case 0x48: ::snprintf(Line.data(), Line.size(), "Sound Controller 3 (Release Time) %s", (Value < 64 ? "shorter" : "longer")); break;       // Controls release time of the Voltage controlled Amplifier (VCA). Default = Release Time.
        case 0x49: ::snprintf(Line.data(), Line.size(), "Sound Controller 4 (Attack Time) %s", (Value < 64 ? "shorter" : "longer")); break;        // Controls the “Attack’ of a sound. The attack is the amount of time it takes forthe sound to reach maximum amplitude.
        case 0x4A: ::snprintf(Line.data(), Line.size(), "Sound Controller 5 (Brightness) %s", (Value < 64 ? "lower" : "higher")); break;           // Controls VCF's cutoff frequency of the filter.
        case 0x4B: ::snprintf(Line.data(), Line.size(), "Sound Controller 6 (Decay Time) %s", (Value < 64 ? "shorter" : "longer")); break;         // Generic – Some manufacturers may use to further shave their sounds.
        case 0x4C: ::snprintf(Line.data(), Line.size(), "Sound Controller 7 (Vibrato Rate) %s", (Value < 64 ? "slower" : "faster")); break;        // Generic – Some manufacturers may use to further shave their sounds.
        case 0x4D: ::snprintf(Line.data(), Line.size(), "Sound Controller 8 (Vibrato Depth) %s", (Value < 64 ? "reduced" : "increased")); break;   // Generic – Some manufacturers may use to further shave their sounds.
        case 0x4E: ::snprintf(Line.data(), Line.size(), "Sound Controller 9 (Vibrato Delay) %s", (Value < 64 ? "shorter" : "longer")); break;      // Generic – Some manufacturers may use to further shave their sounds.
        ...
        case 0x5B: ::snprintf(Line.data(), Line.size(), "Effect 1 Depth (Reverb) Send Level %d", Value); break;
        case 0x5C: ::snprintf(Line.data(), Line.size(), "Effect 2 Depth (Tremolo) Send Level %d", Value); break;
        case 0x5D: ::snprintf(Line.data(), Line.size(), "Effect 3 Depth (Chorus) Send Level %d", Value); break;
        ...
        case 0x64: ::snprintf(Line.data(), Line.size(), "Select RPN parameter (LSB) %d", Value); break;
        case 0x65: ::snprintf(Line.data(), Line.size(), "Select RPN parameter (MSB) %d", Value); break;
