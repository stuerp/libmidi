...
bool processor_t::IsMDS(std::vector<uint8_t> const & data) noexcept
{
    if (data.size() < 8)
        return false;
...
bool processor_t::ProcessMDS(std::vector<uint8_t> const & data, container_t & container)
{
    ...
    // Format chunk
    {
        uint32_t FormatSize = (uint32_t) (it[0] | (it[1] << 8) | (it[2] << 16) | (it[3] << 24)); it += 4;
        ...
        if (FormatSize >= 4)
        {
            ...
            TimeFormat = (uint32_t) (it[0] | (it[1] << 8) | (it[2] << 16) | (it[3] << 24)); it += 4;
            FormatSize -= 4;
        ...
        if (FormatSize >= 4)
        {
            ...
            Flags = (uint32_t) (it[0] | (it[1] << 8) | (it[2] << 16) | (it[3] << 24)); it += 4;
            FormatSize -= 4;
        }
    ...
    uint32_t SegmentCount = (uint32_t) (it[0] | (it[1] << 8) | (it[2] << 16) | (it[3] << 24)); it += 4;
    ...
    for (uint32_t i = 0; i < SegmentCount; ++i)
    {
        ...
        while ((it != SegmentEnd) && (it != BodyEnd))
        {
            ...
            uint32_t Delta = (uint32_t) (it[0] | (it[1] << 8) | (it[2] << 16) | (it[3] << 24)); it += 4;
            ...
            uint32_t Event = (uint32_t) (it[0] | (it[1] << 8) | (it[2] << 16) | (it[3] << 24)); it += 4;
            ...
            if ((Event >> 24) == 0x01)
            {
                ...
                container.AddEventToTrack(0, event_t(Timestamp, event_t::Extended, 0, Data, sizeof(Data)));
            }
            else
            if ((Event >> 24) == 0x00)
            {
                ...
                if (StatusCode >= 0x08 && StatusCode <= 0x0E)
                {
                    ...
                    Track.AddEvent(event_t(Timestamp, (event_t::event_type_t) (StatusCode - 8), Event & 0x0F, Data, Size));
                }
            }
        }
    }
